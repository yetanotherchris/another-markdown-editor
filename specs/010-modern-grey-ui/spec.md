# Feature Specification: Opencode Desktop UI Appearance

**Feature Branch**: `[010-opencode-ui-appearance]`

**Created**: 2026-08-05

**Status**: Draft

**Input**: User description: "I want to create a new spec - to make the appearance look like the opencode desktop UI. You may need to ask some clarifying questions for this, but this is largely about the hamburger menu, for '+' instead of 'new file', using the same tabbed interface. Next to the hamburger menu there should be an icon for the file explorer, to toggle the explorer on the left visible or not. The same colours as opencode desktop should be used which I can find if needed."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Recognize and Use the Primary Workspace Controls (Priority: P1)

A user opens the application and immediately sees the primary workspace controls styled to match the opencode desktop UI. The hamburger menu, file explorer toggle, tab bar, and new-file button are visually consistent with the reference UI and clearly identifiable.
...
**Independent Test**: A user who has seen the opencode desktop UI can identify the hamburger menu, explorer toggle, tab bar, and + button within 5 seconds of opening the app.
...
3. **Given** the user compares the application to the opencode desktop UI reference, **When** they inspect the colors, icons, and layout, **Then** the two appear visually consistent.
...
**Why this priority**: This is a frequently used action that should be visually compact and consistent with the opencode desktop UI tabbed interface.
...
- What if the user has set a custom color theme? The opencode desktop UI color palette takes precedence for this feature; custom theme overrides may be considered out of scope.
...
- **FR-003**: The tab bar must visually match the opencode desktop tabbed interface: inactive tabs display truncated text with ellipsis, the active tab is rendered as a light gray pill (`#EAEAEA`) with rounded corners containing an edit icon, the filename label, and a close button, and tabs exhibit distinct active/inactive states, hover states, and close affordance.
...
- **FR-006**: The main window UI color palette must match the following opencode desktop reference colors: primary background `#FFFFFF`; secondary/surface background `#F9F9FB`/`#F8F8FA`; active tab pill background `#EAEAEA`; primary text `#1A1A1A`/`#222222`; secondary/muted text `#666666`/`#707070`; borders and dividers `#E5E5E5`/`#ECECEC`; accent orange `#D96B27`; control/button background `#2D2D2D`.
...
- **SC-004**: The application's visual appearance is consistent with the opencode desktop UI reference as judged by a side-by-side comparison.
