...
- [X] T001 Create `src/main/configPath.ts` with `universalConfigDir`,
...
- [X] T002 Rewrite `recentItemsConfigPath()` in `src/main/recentItemsPath.ts`:
...
- [X] T003 Add `migrateConfigToUniversalLocation()` and call it first inside
...
- [X] T004 [US1] Add `tests/main/configPath.test.ts` covering the path values
...
- [X] T005 [US2] Add `tests/e2e/universal-config.spec.ts` launching Electron
      ...
      isolates (US4/FR-010).
- [X] T006 [US2] Run `npx playwright test tests/e2e/universal-config.spec.ts`
      and confirm green.

## Phase 4: Polish

- [X] T007 Run the gates: `npm run lint`, `npm run typecheck`, `npm run test`,
      `npm run test:e2e` (the full existing suite must still pass — the seam
      keeps it isolated).
- [X] T008 Archive the feature (`git mv specs/022-universal-config-path
