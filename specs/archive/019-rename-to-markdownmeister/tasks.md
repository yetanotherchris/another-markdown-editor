...
- [X] T001 Change `name` to `markdownmeister` and `productName` to
      `MarkdownMeister` in `package.json` (contract row 1-2; FR-004/FR-003).
- [X] T002 Regenerate `package-lock.json` so its root `name` matches the new
...
- [X] T003 [P] Update `electron-builder.yml`: `appId: com.yetanotherchris.markdownmeister`,
...
- [X] T004 [P] Change the `<title>` to `MarkdownMeister` in
...
- [X] T005 [US1] Add `tests/e2e/rename.spec.ts` asserting the built app's window
      `document.title` is `MarkdownMeister` (US1 acceptance scenarios; FR-001/010).

### Implementation for User Story 1

- [X] T006 [US1] Run `npx playwright test tests/e2e/rename.spec.ts` and confirm
...
- [X] T007 Update `.github/workflows/build-release.yml`: the upload globs
...
- [X] T008 Update `updatescoop.ps1`: `$fileName`/`$url` →
      ...
      FR-009).
- [X] T009 Update `updatebrew.ps1`: the three `$*File` names, the three URL
...
- [X] T010 Rename `another-markdown-editor.json` → `markdownmeister.json`
...
- [X] T011 Rename `Formula/another-markdown-editor.rb` → `Formula/markdownmeister.rb`
...
- [X] T012 [P] Rename the test seams in `src/main/index.ts` and
...
- [X] T013 [P] Update `src/main/recentItemsPath.ts` (`appData/ame` →
...
- [X] T014 [P] Rename the CSS design tokens `--ame-*` → `--mm-*` in
      `src/renderer/**/*.css` and the token references in
      `tests/e2e/{theme,source,header-bar-shade}.spec.ts` (contract row 32).
- [X] T015 [P] Rename `.ame-spelling-error` → `.mm-spelling-error` and the
...
- [X] T016 [P] Rename the `'ame-…'` temp-dir prefixes → `'mm-…'` in
      `tests/main/**`, `tests/renderer/**`, and `tests/e2e/**` (contract row 35).
- [X] T017 [P] Update the `appData/ame` comments in
...
- [X] T018 Update `README.md`: title, `brew install yetanotherchris/tap/markdownmeister`,
...
- [X] T019 Update the `docs/DESIGN_DECISIONS.md` heading to
      `MarkdownMeister — Design Decisions` (keep the repo-URL input line) (contract
      row 36; FR-014).
- [X] T020 Update `.specify/memory/constitution.md`: title to
      `MarkdownMeister Constitution`, add a PATCH amendment note, bump
      version/last-amended (contract row 38).
- [X] T021 Update the active specs per FR-015 (contract rows 39-40):
      ...
      2026-08-08).
- [X] T022 Run the gates and fix anything they catch: `npm run lint`, `npm run
      typecheck`, `npm run test`, `npm run test:e2e` (SC-006).
- [X] T023 Run the SC-002 verification greps from quickstart.md and confirm the
      only remaining matches are the GitHub repo URL (documented exception) plus
      archived specs / build output / lockfile contents.
- [X] T024 Archive the feature: `git mv specs/019-rename-to-markdownmeister
