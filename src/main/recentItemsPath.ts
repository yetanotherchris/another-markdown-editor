 ...
 *
 * FR-004: on `~/.config` platforms the file MUST be at `~/.config/ame/config.json`.
 ...
 * `appData/ame/config.json` is the conventional per-user config location on
 * every platform.
 *
 * Resolved lazily (never cached at module load) so tests can relocate it with
 * `app.setPath('appData', …)` before the first record.
 *
 * `AME_CONFIG_DIR` is a test/CI seam: when set it names the directory that
 ...
 * without touching the developer's real `~/.config/ame`. Production never sets
