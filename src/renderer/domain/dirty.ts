 ...
 *  text the user sees; a formatted document that is clean in the live-dirty
 *  sense writes the stored raw bytes (a no-edit open/save stays byte-identical);
 *  only a document with real drift writes the Crepe serialization so the edits
 *  are kept. */
export function getContentToSave(doc: DocumentState, getMarkdown: MarkdownAccessor): string {
  ...
  if (isDirtyLive(doc, getMarkdown)) return getMarkdown(doc.id) ?? doc.content
  return doc.content
}
